...
chcp 65001 >nul
cd /d "%~dp0"
title Gerar 1.0.118 — iOS + Android (Aha + retenção + force update)
...
if errorlevel 1 ( pause & exit /b 1 )
call _ego_run_python.bat scripts\wait_and_submit_eas.py queue --ids-file builds-1.0.118.ids.json
if errorlevel 1 ( pause & exit /b 1 )
echo Depois: AGUARDAR-E-SUBMETER-1.0.118.bat
pause
