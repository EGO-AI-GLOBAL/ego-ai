...
def health():
    ...
    payload: dict[str, Any] = {
        "service": "ego-ai-api",
        "ok": True,
        "api_build": "2026-08-21-auth-no-server-auto-refresh",
        "checks": {
...
def auth_refresh():
    ...
    if err:
        low = (err or "").lower()
        # 401 SÓ se o refresh morreu de verdade. Mensagens genéricas / rede
        # → 503 para o app NÃO limpar a sessão (pede senha ao reabrir).
        hard = any(
            k in low
            for k in (
                "sessão expirada",
