...
chcp 65001 >nul
cd /d "%~dp0"
title Gerar 1.0.116 — iOS + Android (AdMob a cada 2 sessões)
...
if errorlevel 1 ( pause & exit /b 1 )
call _ego_run_python.bat scripts\wait_and_submit_eas.py queue --ids-file builds-1.0.116.ids.json
if errorlevel 1 ( pause & exit /b 1 )
echo Depois: AGUARDAR-E-SUBMETER-1.0.116.bat
pause
