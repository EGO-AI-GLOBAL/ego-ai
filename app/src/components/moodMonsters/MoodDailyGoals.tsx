...
export function MoodDailyGoals({
  ...
  const gentleMode = Boolean(care.gentleness?.gentle_mode);

  return (
    <View style={styles.wrap}>
      <View style={styles.head}>
        <Text style={[styles.title, { color: colors.text }]}>
          {gentleMode ? "Missões gentis de hoje" : "Missões de hoje"}
        </Text>
      ...
      {hasSurprise ? (
        <Text style={[styles.surpriseHint, { color: colors.textMuted }]}>
          {gentleMode
            ? "Inclui surpresa gentil — dia difícil permitido 🫂"
            : "Inclui 1 missão surpresa — muda todo dia ✨"}
        </Text>
      ) : gentleMode ? (
        <Text style={[styles.surpriseHint, { color: colors.textMuted }]}>
          Sem pressão — só o que seu corpo aguenta hoje.
        </Text>
      ) : null}
      {congrats ? (
