...
export async function saveDailyCarePetName(
  name: string
): Promise<{ daily_care: DailyCareInfo } | null> {
  try {
    const { data } = await api.post("daily-care/pet-name", { name: name.trim() });
    const body = unwrap<{ daily_care: DailyCareInfo }>(data);
    if (!body.daily_care) return null;
    return body;
  } catch {
...
/** Android/iOS: multipart nativo primeiro; base64 só fallback (Android mais rápido). */
